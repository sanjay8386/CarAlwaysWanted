...
+(void)verifyDeviceID: (NSString *)anDeviceId  completion: (void (^)(id, NSInteger, NSError*))callback {
    if (anDeviceId) {
         NSString *STR22 = [NSString stringWithFormat:@"<tsf><action>requestAuthentication</action><parameters><deviceId>%@</deviceId></parameters></tsf>",anDeviceId];
...
+ (void)getAuthKey:(NSString *)anDeviceId verifykey:(NSString *)anVerifyKey  completion: (void (^)(id, NSInteger, NSError*))callback {
    
    if (anVerifyKey && anDeviceId) {
        
        
        NSString *xmlgetAuth =[NSString stringWithFormat:@"<tsf><action>getAuthKey</action><parameters><deviceId>%@</deviceId><verificationKey><![CDATA[%@]]></verificationKey></parameters></tsf>",anDeviceId,anVerifyKey];
        [self executeAPICall:@"POST" data:xmlgetAuth completion:callback];
    }
}
