...
+ (BOOL)isNotEqualToString:(NSString *)string withstring:(NSString *)string1 {
   
    return [string isEqualToString:string1] ? YES : NO;
}
...
+(BOOL)checkIfBlank:(NSString *)string {
    return (string.length == 0) ? NO : YES;
}

+(BOOL)checkIffirstDigit:(NSString *)string {
    return ([string characterAtIndex:0] == '0')? NO : YES;
}
